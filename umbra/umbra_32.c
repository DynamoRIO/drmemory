...
shadow_table_init_redzone(umbra_map_t *map, byte *block)
{
    ...
    if (map->options.redzone_size != 0) {
        ...
        if (map->options.make_redzone_faulty) {
            ...
            memset(post_redzone, map->options.redzone_value, map->options.redzone_size);
        }
    }
    return block;
}
 ...
 */
static void
shadow_table_delete_special_block(umbra_map_t *map, special_block_t *block)
{
    nonheap_free(block->start - map->options.redzone_size, map->shadow_block_alloc_size,
                 HEAPSTAT_SHADOW);
...
static void
shadow_table_create_special_block_helper(umbra_map_t *map, ptr_uint_t value,
                                         size_t value_size,
                                         special_block_t *special_block)
{
    ...
    block = (byte *)nonheap_alloc(map->shadow_block_alloc_size,
                                  DR_MEMPROT_READ | DR_MEMPROT_WRITE, HEAPSTAT_SHADOW);
    ASSERT(block != NULL && ALIGNED(block, PAGE_SIZE), "fail to alloc special block");
    ...
    IF_DEBUG(ok =)
    dr_memory_protect(block - map->options.redzone_size, map->shadow_block_alloc_size,
                      DR_MEMPROT_READ);
    ASSERT(ok, "-w failed: will have inconsistencies in shadow data");
...
{
    shadow_table_create_special_block_helper(map, map->options.default_value,
                                             map->options.default_value_size,
                                             &map->default_block);
}

static byte *
shadow_table_lookup_special_block(umbra_map_t *map, ptr_uint_t value, size_t value_size)
{
    int i, num_blks;
...
static byte *
shadow_table_create_special_block(umbra_map_t *map, ptr_uint_t value, size_t value_size)
{
    byte *block;
    int num_blks;
                                       ...
                                       reg_id_t reg_addr, reg_id_t reg_idx)
{
    ...
    /* %reg_index >>= 16 */
    PRE(ilist, where,
        INSTR_CREATE_shr(drcontext, opnd_create_reg(reg_idx),
    ...
    if (UMBRA_MAP_SCALE_IS_DOWN(map->options.scale)) {
        /* %reg_addr >>= map->scale*/
        PRE(ilist, where,
            INSTR_CREATE_shr(drcontext, opnd_create_reg(reg_addr),
                             OPND_CREATE_INT8(map->shift)));
    } else if (UMBRA_MAP_SCALE_IS_UP(map->options.scale)) {
        /* %reg_addr <<= map->scale*/
        PRE(ilist, where,
            INSTR_CREATE_shl(drcontext, opnd_create_reg(reg_addr),
                             OPND_CREATE_INT8(map->shift)));
    }
    ...
    PRE(ilist, where,
        INSTR_CREATE_add(
            drcontext, opnd_create_reg(reg_addr),
            opnd_create_base_disp(REG_NULL, reg_idx, sizeof(ptr_int_t), disp, OPSZ_PTR)));
}
#elif defined(ARM)
/* code sequence:
 * %reg_idx   = table
                                       ...
                                       reg_id_t reg_addr, reg_id_t reg_idx)
{
    ...
    instrlist_insert_mov_immed_ptrsz(drcontext, (uint)(map->shadow_table),
                                     opnd_create_reg(reg_idx), ilist, where, NULL, NULL);

    /* %reg_idx += (%reg_addr >> 14) */
    PRE(ilist, where,
        INSTR_CREATE_add_shimm(drcontext, opnd_create_reg(reg_idx),
                               opnd_create_reg(reg_idx), opnd_create_reg(reg_addr),
                               OPND_CREATE_INT(DR_SHIFT_LSR), OPND_CREATE_INT(14)));

    /* Can't specify a bitmask of 0xfffffffc, so instead we use its inverse, BIC.
     * %reg_idx &= 0xfffffffc
     */
    PRE(ilist, where,
        INSTR_CREATE_bic(drcontext, opnd_create_reg(reg_idx), opnd_create_reg(reg_idx),
    ...
    if (UMBRA_MAP_SCALE_IS_DOWN(map->options.scale)) {
        PRE(ilist, where,
            INSTR_CREATE_add_shimm(drcontext, opnd_create_reg(reg_addr),
                                   opnd_create_reg(reg_idx), opnd_create_reg(reg_addr),
...
static bool
shadow_table_is_in_default_block(umbra_map_t *map, byte *shadow_addr, bool *redzone OUT)
{
...
static bool
shadow_table_is_in_special_block(umbra_map_t *map, byte *shadow_addr, ptr_uint_t *value,
                                 size_t *value_size, bool *redzone OUT)
{
    uint i;
    for (i = 0; i < map->num_special_blocks; i++) {
        if (shadow_addr >= (map->special_blocks[i].start - map->options.redzone_size) &&
            shadow_addr < (map->special_blocks[i].start + map->shadow_block_size +
                           map->options.redzone_size)) {
            if (value != NULL)
...
static bool
shadow_table_use_special_block(umbra_map_t *map, app_pc app_addr, ptr_uint_t *value,
                               size_t *value_size)
{
    return shadow_table_is_in_special_block(
        map, shadow_table_app_to_shadow(map, app_addr), value, value_size, NULL);
}
...
shadow_table_is_in_normal_block(umbra_map_t *map, byte *shadow_addr)
{
    return (!shadow_table_is_in_default_block(map, shadow_addr, NULL) &&
            !shadow_table_is_in_special_block(map, shadow_addr, NULL, NULL, NULL));
}
...
shadow_table_replace_block(umbra_map_t *map, app_pc app_base)
{
    ...
    if (shadow_table_use_default_block(map, app_base) ||
        ...
        ASSERT(value <= USHRT_MAX && value_size == 1, "value_size > 1 is not supported");
...
shadow_table_init(umbra_map_t *map)
{
    ...
    if (!TEST(UMBRA_MAP_CREATE_SHADOW_ON_TOUCH, map->options.flags)) {
        ...
        byte *start = shadow_table_create_special_block(map, map->options.default_value,
                                                        map->options.default_value_size);
        for (i = 0; i < SHADOW_TABLE_ENTRIES; i++)
...
umbra_map_arch_init(umbra_map_t *map, umbra_map_options_t *ops)
{
    ...
    map->shadow_block_alloc_size = map->shadow_block_size + 2 * map->options.redzone_size;
...
drmf_status_t
umbra_create_shadow_memory_arch(umbra_map_t *map, uint flags, app_pc app_addr,
                                size_t app_size, ptr_uint_t value, size_t value_size)
{
    ...
    umbra_map_lock(map);
    APP_RANGE_LOOP(
        app_addr, app_size, app_blk_base, app_blk_end, app_src_end, start, end, iter_size,
        {
            if (shadow_table_use_default_block(map, app_blk_base)) {
                ...
                if (TEST(flags, UMBRA_CREATE_SHADOW_SHARED_READONLY) &&
                     ...
                     */
                    shadow_blk =
                        shadow_table_create_special_block(map, value, value_size);
                    if (shadow_blk != NULL) {
                        shadow_table_set_block(map, SHADOW_TABLE_INDEX(app_blk_base),
                                               shadow_blk);
                        continue;
                    }
                }
            ...
            res = umbra_shadow_set_range_arch(map, start, iter_size, &size, value,
                                              value_size);
            if (res != DRMF_SUCCESS) {
...
drmf_status_t
umbra_delete_shadow_memory_arch(umbra_map_t *map, app_pc app_addr, size_t app_size)
{
    size_t shadow_size;
...
drmf_status_t
umbra_read_shadow_memory_arch(IN umbra_map_t *map, IN app_pc app_addr, IN size_t app_size,
                              INOUT size_t *shadow_size, IN byte *buffer)
{
    /* i#1260: end pointers are all closed (i.e., inclusive) to handle overflow */
    app_pc app_blk_base, app_blk_end, app_src_end;
    app_pc start, end;
    ...
    if (POINTER_OVERFLOW_ON_ADD(app_addr, app_size - 1)) /* just hitting top is ok */
        return DRMF_ERROR_INVALID_SIZE;
    shdw_size = 0;
    APP_RANGE_LOOP(app_addr, app_size, app_blk_base, app_blk_end, app_src_end, start, end,
                   iter_size, {
...
drmf_status_t
umbra_write_shadow_memory_arch(IN umbra_map_t *map, IN app_pc app_addr,
                               IN size_t app_size, INOUT size_t *shadow_size,
                               IN byte *buffer)
{
    /* i#1260: end pointers are all closed (i.e., inclusive) to handle overflow */
    app_pc app_blk_base, app_blk_end, app_src_end;
    ...
    APP_RANGE_LOOP(
        app_addr, app_size, app_blk_base, app_blk_end, app_src_end, start, end, iter_size,
        {
            ...
            if (shadow_table_is_in_default_block(map, shadow_start, NULL))
                return DRMF_ERROR_INVALID_PARAMETER;
            if (shadow_table_is_in_special_block(map, shadow_start, NULL, NULL, NULL)) {
                shadow_table_replace_block(map, app_blk_base);
                shadow_start = shadow_table_app_to_shadow(map, start);
            }
            size = umbra_map_scale_app_to_shadow(map, iter_size);
...
drmf_status_t
umbra_shadow_set_range_arch(IN umbra_map_t *map, IN app_pc app_addr, IN size_t app_size,
                            OUT size_t *shadow_size, IN ptr_uint_t value,
    ...
    APP_RANGE_LOOP(app_addr, app_size, app_blk_base, app_blk_end, app_src_end, start, end,
                   iter_size, {
                       ...
                       if (shadow_table_is_in_default_block(map, shadow_start, NULL))
                           return DRMF_ERROR_INVALID_PARAMETER;
                       size = umbra_map_scale_app_to_shadow(map, iter_size);
                       if (shadow_table_is_in_special_block(map, shadow_start, &blk_val,
                                                            &blk_val_sz, NULL)) {
                           shadow_table_replace_block(map, app_blk_base);
                           shadow_start = shadow_table_app_to_shadow(map, start);
                       }
...
drmf_status_t
umbra_shadow_copy_range_arch(IN umbra_map_t *map, IN app_pc app_src, IN app_pc app_dst,
                             IN size_t app_size_in, OUT size_t *shadow_size_out)
{
    /* i#1260: end pointers are all closed (i.e., inclusive) to handle overflow */
    app_pc app_blk_base, app_blk_end, app_src_end;
    app_pc start, end;
    ...
    if (app_src < app_dst && app_src + (app_sz - 1) >= app_dst) {
        ...
        if (umbra_read_shadow_memory_arch(map, app_dst, tail_size, &shadow_sz,
                                          overlap_tail) != DRMF_SUCCESS)
    ...
    tot_shadow_sz = 0;
    APP_RANGE_LOOP(
        app_src, app_sz, app_blk_base, app_blk_end, app_src_end, start, end, iter_size, {
            shadow_start = shadow_table_app_to_shadow(map, start);
            if (shadow_table_is_in_default_block(map, shadow_start, NULL)) {
            ...
            res = umbra_write_shadow_memory_arch(map, app_dst, iter_size, &shadow_sz,
                                                 shadow_start);
            if (res != DRMF_SUCCESS) {
    ...
    if (overlap_tail != NULL) {
        if (res == DRMF_SUCCESS) {
            shadow_sz = umbra_map_scale_app_to_shadow(map, tail_size);
            res = umbra_write_shadow_memory_arch(map, app_dst + (app_dst - app_src),
                                                 tail_size, &shadow_sz, overlap_tail);
            tot_shadow_sz += shadow_sz;
        }
...
drmf_status_t
umbra_value_in_shadow_memory_arch(IN umbra_map_t *map, INOUT app_pc *app_addr,
                                  ...
                                  IN size_t value_size, OUT bool *found)
{
    ...
    *found = false;
    APP_RANGE_LOOP(
        *app_addr, app_size, app_blk_base, app_blk_end, app_src_end, start, end,
        iter_size, {
            if (shadow_table_use_default_block(map, app_blk_base))
                return DRMF_ERROR_INVALID_PARAMETER;
...
drmf_status_t
umbra_insert_app_to_shadow_arch(void *drcontext, umbra_map_t *map, instrlist_t *ilist,
                                instr_t *where, reg_id_t addr_reg, reg_id_t *scratch_regs,
                                int num_scratch_regs)
{
    if (num_scratch_regs < umbra_num_scratch_regs_for_translation_arch() ||
        scratch_regs == NULL)
        return DRMF_ERROR_INVALID_PARAMETER;
    shadow_table_insert_app_to_shadow_arch(drcontext, map, ilist, where, addr_reg,
                                           scratch_regs[0]);
    return DRMF_SUCCESS;
}

drmf_status_t
umbra_iterate_shadow_memory_arch(umbra_map_t *map, void *user_data,
                                 shadow_iterate_func_t iter_func)
{
    ...
    for (i = 0; i < SHADOW_TABLE_ENTRIES; i++) {
        ...
        if (shadow_table_is_in_special_block(map, info.shadow_base, NULL, NULL, NULL))
...
drmf_status_t
umbra_shadow_memory_is_shared_arch(IN umbra_map_t *map, IN byte *shadow_addr,
                                   OUT umbra_shadow_memory_type_t *shadow_type)
{
    bool redzone;
    if (shadow_table_is_in_special_block(map, shadow_addr, NULL, NULL, &redzone)) {
...
drmf_status_t
umbra_get_shadow_memory_type_arch(umbra_map_t *map, byte *shadow_addr,
...
drmf_status_t
umbra_get_shadow_memory_arch(umbra_map_t *map, app_pc app_addr, byte **shadow_addr,
                             umbra_shadow_memory_info_t *shadow_info)
{
    if (shadow_addr != NULL)
        *shadow_addr = shadow_table_app_to_shadow(map, app_addr);
    if (shadow_info != NULL) {
        shadow_info->app_base = (app_pc)ALIGN_BACKWARD(app_addr, map->app_block_size);
        shadow_info->app_size = map->app_block_size;
        shadow_info->shadow_size = map->shadow_block_size;
        shadow_info->shadow_base = shadow_table_app_to_shadow(map, shadow_info->app_base);
        if (shadow_table_use_default_block(map, app_addr)) {
            shadow_info->shadow_type = UMBRA_SHADOW_MEMORY_TYPE_SHADOW_NOT_ALLOC;
            return DRMF_SUCCESS;
        }
...
drmf_status_t
umbra_replace_shared_shadow_memory_arch(umbra_map_t *map, app_pc app_addr,
                                        byte **shadow_addr)
{
...
drmf_status_t
umbra_create_shared_shadow_block_arch(IN umbra_map_t *map, IN ptr_uint_t value,
                                      IN size_t value_size, OUT byte **block)
{
...
drmf_status_t
umbra_get_shared_shadow_block_arch(IN umbra_map_t *map, IN ptr_uint_t value,
                                   IN size_t value_size, OUT byte **block)
{
    uint i;
    for (i = 0; i < map->num_special_blocks; i++) {
